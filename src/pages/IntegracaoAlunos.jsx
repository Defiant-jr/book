...
const resolveImportMode = ({ fileName, sheetName }) => {
  ...
  if (names.includes('alunos angra')) {
    return { label: 'Alunos Angra', unidade: 'CNA Angra dos Reis', shouldClearTable: true };
  }
  if (names.includes('alunos manga')) {
    return { label: 'Alunos Manga', unidade: 'CNA Mangaratiba', shouldClearTable: false };
  }
  return null;
};
...
const REQUIRED_HEADERS = ['nome_do_aluno', 'nome_do_responsavel_financeiro', 'turma'];

const mapRowsToAlunos = (rows, headerIndex, mode) => {
  ...
  for (const row of dataRows) {
    ...
    const aluno = normalizeCellValue(row[headerIndex.nome_do_aluno]);
    const responsavel = normalizeCellValue(row[headerIndex.nome_do_responsavel_financeiro]);
    const turma = normalizeCellValue(row[headerIndex.turma]);

    const hasValue = [aluno, responsavel, turma].some((value) => value != null && value !== '');
...
const IntegracaoAlunos = () => {
  const INTEGRACAO_REF = 62400;
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (!mode) {
        throw new Error('Use uma planilha chamada "Alunos Angra" ou "Alunos Manga".');
      }
      ...
      const entries = mapRowsToAlunos(rows, headerIndex, mode);
      ...
      if (mode.shouldClearTable) {
        const { error: deleteError } = await supabase
          ...
          .not('id', 'is', null);
        if (deleteError) {
          throw new Error(deleteError.message || 'Erro ao limpar alunos.');
        }
      }
      ...
      for (const chunk of chunks) {
        ...
        if (error) {
          throw new Error(error.message || 'Erro ao inserir alunos.');
        }
      }

      toast({
        title: 'Importacao concluida',
        description: `${entries.length} alunos importados de ${mode.label}.`,
      });
    } catch (error) {
      toast({
  ...
  return (
    ...
    >
      <Helmet>
        <title>Integracao Alunos - SysFina</title>
        <meta name="description" content="Importacao de alunos via planilha Excel." />
      </Helmet>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-sm text-gray-300">Importe uma planilha Excel para popular a tabela alunos.</span>
          </div>
        </div>
      ...
      <Card className="glass-card">
        ...
        <CardContent className="space-y-6">
          <div className="space-y-2">
            <label className="text-sm text-gray-300" htmlFor="alunos-file">
          ...
          <div className="text-xs text-gray-400 space-y-1">
            <div>Mapeamento: Nome do Aluno -> aluno, Nome do Responsavel Financeiro -> responsavel, Turma -> turma.</div>
            <div>Use uma planilha chamada "Alunos Angra" ou "Alunos Manga".</div>
            <div>"Alunos Angra" apaga todo o conteudo da tabela alunos antes de importar.</div>
            <div>"Alunos Manga" somente adiciona os registros.</div>
          </div>
        </CardContent>
      </Card>
    </motion.div>
...
export default IntegracaoAlunos;
