...
const OperacoesCadastroRateio = () => {
  ...
  const rateiosOrdenados = useMemo(() => {
    if (!rateios.length || !ordenacao) return rateios;
    return [...rateios].sort((a, b) => {
  ...
  const fetchRateios = async () => {
    ...
    if (error) {
      toast({
        ...
        description: error.message || 'Nao foi possivel buscar os rateios.',
        variant: 'destructive',
      });
      return;
    }
  ...
  const handleSave = async () => {
    ...
    const updates = Array.from(dirtyIds)
      .map((id) => {
        const row = rateios.find((item) => item.id === id);
        if (!row) {
          return null;
        }
        ...
        return supabase.from('rateio').update(payload).eq('id', id);
      })
      .filter(Boolean);
    ...
    toast({ title: 'Alteracoes salvas', description: 'Os dados de rateio foram atualizados.' });
    setDirtyIds(new Set());
  };
  ...
  return (
    ...
    >
      <Helmet>
        <title>Cadastro de Rateio - Operacoes</title>
        <meta name="description" content="Edicao da tabela de rateio." />
      </Helmet>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <h1 className="text-3xl font-bold gradient-text">Rateio</h1>
      ...
      <div className="space-y-4">
        {loading && <p className="text-gray-300">Carregando rateios...</p>}
...
export default OperacoesCadastroRateio;
